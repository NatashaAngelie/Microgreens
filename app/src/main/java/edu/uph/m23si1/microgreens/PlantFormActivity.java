...
public class PlantFormActivity extends AppCompatActivity {
    ...
    private void savePlant() {
        ...
        Task<Void> task;
        if (plantId == null) {
            task = plantsRef.push().setValue(payload);
        } else {
            task = plantsRef.child(plantId).setValue(payload);
        }

        task.addOnCompleteListener(PlantFormActivity.this, t -> {
            if (t.isSuccessful()) {
                Intent back = new Intent(PlantFormActivity.this, MainActivity.class);
                ...
                String msg = e != null && e.getMessage() != null
                        ? e.getMessage()
                        : getString(R.string.save_plant_failed);
                Toast.makeText(PlantFormActivity.this, msg, Toast.LENGTH_LONG).show();
            }
        });
    }
